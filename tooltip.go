...
//	proton.Tooltip(win, &hov, "Saves the file", func(win proton.Context) {
//	    proton.Button(win, &saveBtn, "Save")
//	})
func Tooltip(win Context, state *Clickable, tip string, content func(Context)) {
	win.add(func(gtx layout.Context) layout.Dimensions {
		dims := state.Layout(gtx, child(win, content))
		if !state.Hovered() {
			return dims
		}
		lbl := material.Caption(win.theme(), tip)

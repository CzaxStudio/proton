...
func TextView(win Context, state *widget.List, text string) {
	lines := strings.Split(text, "\n")
	win.add(func(gtx layout.Context) layout.Dimensions {
		state.Axis = layout.Vertical
		return material.List(win.theme(), state).Layout(gtx, len(lines), func(gtx layout.Context, i int) layout.Dimensions {
			return layout.UniformInset(unit.Dp(2)).Layout(gtx, func(gtx layout.Context) layout.Dimensions {
				lbl := material.Body2(win.theme(), lines[i])
...
func LogView(win Context, state *widget.List, text string) {
	lines := strings.Split(text, "\n")
